globals [ max-sheep wolve-prey-count my-energy sheep-subjected wolves1]  ; don't let sheep population grow too large
; Sheep and wolves are both breeds of turtle.
breed [ sheep1 a-sheep1 ]  ; sheep is its own plural, so we use "a-sheep" as the singular.
turtles-own [ energy ]       ; both wolves and sheep have energy
patches-own [ g-energy ]

to setup
  clear-all
  ask patches [
    set g-energy random 100  ; assign grass energy
    ifelse g-energy > 20 [set pcolor green] [set pcolor brown] ; set color indicator
  ]
  create-sheep1 initial-number-sheep  ; create the sheep, then initialize their variables
  [
    set shape "sheep"
    set color white
    set size 1  ; easier to see
    set energy 100 ;random (2 * sheep-gain-from-food)
...
to go
  ask sheep1 [
    ifelse energy <= roaming-limit [move] [reproduce-sheep] ;either move or stay put and potentially reproduce
...
to eat-grass  ; sheep procedure
  if g-energy > 20 [
    if energy < 100 [
      ...
      set g-energy g-energy - (2 * sheep-gain-from-food)
    ]
  ]
end

to reproduce-sheep  ; sheep procedure
  if energy > 50 AND random-float 100 < sheep-reproduce [  ; throw "dice" to see if you will reproduce
...
to eat  ; wolf procedure
  ...
  let prey sheep1-on neighbors  ; define prey as all agents in the not von-nuemann neighborhood
  set prey prey with [ energy < my-energy ] ; set the prey list to only weaker ones
  set energy energy + (winner-takes * (count prey) / 2) ; all the weaker ones if you energy
  ask prey [ set energy energy - (winner-takes * 2) ] ; all the weaker ones are made weaker
  ifelse count prey > 1 AND count prey = sum [count sheep1-here] of neighbors
  [ set color black ] ; if the sheep is king of the neighbors then set as black. Else set as white
  [ set color white ]
...
"" ""
PENS
"sheep" 1.0 0 -612749 true "" "plot count sheep1"
"kings" 1.0 0 -16449023 true "" "plot count sheep1 with [energy = 100]"
"grass / 4" 1.0 0 -10899396 true "" "[ plot sum g-energy ]"
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="winner-takes" first="2" step="1" last="10"/>
    ...
    <enumeratedValueSet variable="roaming-limit">
      <value value="31"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="war">
      <value value="true"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
